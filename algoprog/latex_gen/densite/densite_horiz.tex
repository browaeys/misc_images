...
\begin{axis}[
    ...
    \addplot [thick, black, samples=1000, domain=-5:5.2] {densite(x)} 
        node[right, pos=0.7, xshift=8pt, yshift=5pt] {$p_X(t)$}; % f(t) repositionné
    
    \draw [thin] (axis cs:0.8, 0) -- (axis cs:0.8, {densite(0.8)});
    
    \node (eq1) at (axis cs:-1.5, 1.3) {$\displaystyle F_X(x) = \mathbb{P}(X \leq x) = \int_{-\infty}^{x} p_X(t) \, dt$};
...
\begin{axis}[
    ...
    \addplot [thick, black, samples=1000, domain=-5:5.2] {densite(x)} 
        node[right, pos=0.7, xshift=8pt, yshift=5pt] {$p_X(x)$}; % f(t) repositionné
    
    \draw [thin] (axis cs:-0.8, 0) -- (axis cs:-0.8, {densite(-0.8)});
    \draw [thin] (axis cs:1.2, 0) -- (axis cs:1.2, {densite(1.2)});
    
    \node (eq2) at (axis cs:-1, 1.3) {$\displaystyle \mathbb{P}(a < X \leq b) = F_X(b) - F_X(a) = \int_{a}^{b} p_X(x) \, dx$};
