...
for ax in axes:
    ...
    ax.text(-.035,2.54,'$u(t)$',ha='center',va='bottom',fontsize=18)
    ...
    ax.text(-.09,mean,r'$\bar{U}$',color=magenta,ha='right',va='center',fontsize=19)

axes[0].fill_between(inside,0,signal(inside),color=blue,alpha=.18,zorder=0)
axes[0].plot(t,signal(t),color=blue,lw=2.5)
axes[0].text(.69,.35,r'$\int_{t_0}^{t_0+T} u(t)\,\mathrm{d}t$',color=blue,ha='center',fontsize=18)
...
axes[1].text(.68,.58,r'$T\,\bar{U}$',color=magenta,ha='center',fontsize=23)
